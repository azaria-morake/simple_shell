...
int builtin_finder(info_t *obj1)
{
  ...
  for (x = 0; builtin_tbl[x].c_type; x++)
    if (c_strcmp(obj1->arg_v[0], builtin_tbl[x].c_type) == 0)
      {
	obj1->ln_count++;
	built_in_return = builtin_tbl[x].function_(obj1);
	break;
      }
  ...
  if (passage)
  ...
  else
    {
      if ((reactive(obj1) || habitat2(obj1, "PATH=")
	   || obj1->arg_v[0][0] == '/') && cmd(obj1, obj1->arg_v[0]))
	cmd_fork(obj1);
      else if (*(obj1->ar_g) != '\n')
	{
	  obj1->condition = 127;
