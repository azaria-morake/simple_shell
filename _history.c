...
int record_hist(info_t *info)
{
  ...
  if (fd == -1)
    return (-1);
  for (node = info->hist_node; node; node = node->nxt)
    {
      c_putsfd(node->str, fd);
      c_put('\n', fd);
    }
  c_put(BUFFER_FLUSH, fd);
  close(fd);
  return (1);
}
...
int rd_hist(info_t *info)
{
  ...
  for (i = 0; i < fsize; i++)
    if (buf[i] == '\n')
      {
	buf[i] = 0;
	create_hist_ls(info, buf + last, linecount++);
	last = i + 1;
      }
  if (last != i)
    create_hist_ls(info, buf + last, linecount++);
  free(buf);
  info->count_hist = linecount;
...
int create_hist_ls(info_t *info, char *buf, int linecount)
{
  ...
  if (info->hist_node)
    node = info->hist_node;
  app_node_term(&node, buf, linecount);
